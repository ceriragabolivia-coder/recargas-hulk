        ...
        >
          <div style={{ position: 'relative', maxWidth: '100%', maxHeight: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <button 
              ...
              onClick={() => setShowGuideModal(false)}
            >✕</button>
            <img loading="lazy" decoding="async" src={selectedJuego.guia_id_url} 
              alt="Guía de ID" 
              style={{ 
        ...
        <div className="game-info-col" style={{ flex: '1 1 280px', maxWidth: '350px', display: 'flex', flexDirection: 'column', gap: '2px' }}>
          ...
          <div className="card" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', padding: '6px 12px', gap: '0px' }}>
            {selectedJuego.icono_url ? (
              <img loading="lazy" decoding="async" src={getOptimizedImageUrl(selectedJuego.icono_url, 300)} alt={selectedJuego.nombre} style={{ width: 110, height: 110, minWidth: 110, minHeight: 110, flexShrink: 0, objectFit: 'cover', borderRadius: '16px', filter: 'drop-shadow(0 8px 16px rgba(0,0,0,0.4))', backgroundColor: 'rgba(255,255,255,0.02)' }} />
            ) : (
          ...
          {selectedJuego.tutorial_video_url && (
            ...
            >
              {selectedJuego.tutorial_banner_img ? (
                <img loading="lazy" decoding="async" src={getOptimizedImageUrl(selectedJuego.tutorial_banner_img, 600)} alt="Tutorial" style={{ width: '100%', display: 'block' }} />
              ) : (
          ...
          {config?.promo_banner_texto && (
            ...
            >
              {config.promo_banner_icono_url ? (
                <img loading="lazy" decoding="async" src={getOptimizedImageUrl(config.promo_banner_icono_url, 100)} alt="Promo" style={{ width: 32, height: 32, objectFit: 'contain' }} />
              ) : (
                 <div style={{ fontSize: '20px' }}>🎁</div>
              )}
              <div style={{ flex: 1 }}>
      ...
      <div className="catalogo-grid">
        {filteredJuegosData.map(juego => {
          ...
          return (
            ...
            >
              {juego.icono_url ? (
                <img loading="lazy" decoding="async" src={getOptimizedImageUrl(juego.icono_url, 200)} alt={juego.nombre} style={{ width: 84, height: 84, objectFit: 'cover', borderRadius: '20px', boxShadow: '0 4px 12px rgba(0,0,0,0.25)' }} />
              ) : (
