...
const DEFAULT_CANCEL_MESSAGE = (
  num
) => `Tu Pedido #${num} se ha cancelado motivado a que la referencia de pago que colocaste no ha podido ser encontrado en nuestro banco, es decir, el pago no pudo ser verificado y esto se debe a alguno de los siguientes motivos:
...
export default function Pedidos({
  ...
  const isEmpleado =
    perfil?.rol?.toLowerCase() === "empleado" ||
    perfil?.rol?.toLowerCase() === "trabajador";
  ...
  const refreshPedidoData = async (pedidoId) => {
    ...
    if (updData) {
      setSelectedPedido((prev) => {
        if (prev?.id === pedidoId) {
          return {
            ...
            atendido_por: prev.atendido_por,
          };
        }
        return prev;
      });
      setPedidos((prev) =>
        prev.map((p) =>
          p.id === pedidoId
            ? { ...updData, cliente: p.cliente, atendido_por: p.atendido_por }
            : p
        )
      );
    }
  ...
  const motivosRechazoArray = useMemo(() => {
    ...
    return [
      {
        id: "1",
        titulo: "ID de Jugador Inválido",
        mensaje: "El ID proporcionado no corresponde a ninguna cuenta válida.",
      },
      {
        id: "2",
        titulo: "Región Incorrecta",
        mensaje:
          "El ID proporcionado pertenece a otra región. Por favor verifica los datos de tu cuenta.",
      },
      {
        id: "3",
        titulo: "Cuenta No Encontrada",
        mensaje:
          "No pudimos encontrar una cuenta con el ID y Nickname proporcionados en este juego.",
      },
      {
        id: "4",
        titulo: "Datos de Cuenta Incorrectos",
        mensaje:
          "El usuario y/o contraseña proporcionados para el acceso a la cuenta son incorrectos.",
      },
      {
        id: "5",
        titulo: "Problema con la Transacción",
        mensaje:
          "El pago ha sido revertido o se ha detectado un problema con la transacción, no se puede realizar la recarga.",
      },
      {
        id: "6",
        titulo: "Paquete No Disponible",
        mensaje:
          "El paquete solicitado no está disponible temporalmente para recargas en esta región.",
      },
      {
        id: "7",
        titulo: "Mantenimiento del Juego",
        mensaje:
          "Mantenimiento en los servidores del juego. Por favor intenta crear un nuevo pedido más tarde.",
      },
    ];
  ...
  const [filtroEstado, setFiltroEstado] = useState(
    incomingFilterKey === "ordenes_pendientes"
      ? "pendiente"
      : incomingFilterKey || "todos"
  );
    ...
    if (busqueda && busqueda.trim()) {
      ...
      if (isNum) {
        ...
        const { data: matchedClientes } = await supabase
          ...
          .select("auth_user_id")
          .or(
            `nombres.ilike.%${cleanQ}%,nickname.ilike.%${cleanQ}%,usuario.ilike.%${cleanQ}%`
          )
          .limit(50);
      ...
      const fetchStock = async () => {
        ...
        for (const item of selectedPedido.pedido_items) {
          const prod =
            item.productos ||
            item.producto ||
            (Array.isArray(item.productos) ? item.productos[0] : null);
          if (
            prod?.entrega_automatica &&
            !item.codigo_entregado &&
            counts[prod.id] === undefined
          ) {
      ...
      const findAndOpen = () => {
        if (targetOrderId) {
          ...
          const order = pedidos.find(
            (p) =>
              String(p.numero_pedido).padStart(6, "0") === cleanNum ||
              String(p.numero_pedido) === cleanNum
          );
          if (order) {
            setSelectedPedido(order);
            return true;
  ...
  useEffect(() => {
    if (selectedPedido) {
      ...
      setCancelacionMensaje(
        DEFAULT_CANCEL_MESSAGE(selectedPedido.numero_pedido || "")
      );
    }
  }, [selectedPedido?.id]);
    ...
    async function fetchClientData() {
      if (
        ...
        const { data, error } = await supabase
          ...
          .or(
            `id.eq.${selectedPedido.cliente_id},auth_user_id.eq.${selectedPedido.cliente_id}`
          )
          .maybeSingle();
          ...
          setSelectedPedido((prev) => {
            if (prev?.id === selectedPedido.id) {
              const isAdminFallback =
                user && selectedPedido.cliente_id === user.id;
              ...
              return {
                ...
                cliente: isAdminFallback
                  ? {
                      auth_user_id: user.id,
                      nombres:
                        user.user_metadata?.nombres || "Tú (Administrador)",
                      apellidos: user.user_metadata?.apellidos || "",
                      usuario: user.email,
                      nickname: user.user_metadata?.nickname || "Admin",
                      whatsapp:
                        user.user_metadata?.whatsapp || "No especificado",
                    }
                  : {
  ...
  const filterPedidos = (list, key) => {
    ...
    if (key === "pagos_pendientes") {
      return list.filter(
        (p) =>
          p.pago_verificado === null &&
          !["completado", "cancelado", "reembolsado"].includes(p.estado)
      );
    }
    if (key === "recargas_pendientes") {
      return list.filter(
        (p) =>
          p.pago_verificado === true &&
          !["completado", "cancelado", "reembolsado"].includes(p.estado)
      );
    }
    ...
    pedidosFiltrados = pedidosFiltrados.filter((p) => {
      ...
      const matchCliente =
        ...
        String(p.cliente?.whatsapp || "")
          .toLowerCase()
          .includes(q);
      const matchRef = String(p.referencia_pago || "")
        .toLowerCase()
        .includes(q);
      const matchItems = (p.pedido_items || []).some(
        (item) =>
          ...
          String(item.account_email || "")
            .toLowerCase()
            .includes(q) ||
          String(item.account_password || "")
            .toLowerCase()
            .includes(q) ||
          String(item.datos_extra || "")
            .toLowerCase()
            .includes(q)
      );
  ...
  const updateEstado = async (pedidoId, nuevoEstado) => {
    ...
    if (fetchError || !pedidoActual) {
      ...
      showAlert(
        "No se pudieron obtener los detalles del pedido para completar la operación. " +
          (fetchError?.message || ""),
        "error"
      );
      return;
    ...
    if (nuevoEstado === "completado") {
      for (const item of pedidoActual.pedido_items || []) {
        ...
        if (
          codeInput &&
          codeInput.value &&
          codeInput.value !== (item.codigo_entregado || "")
        ) {
        ...
        if (
          refInput &&
          refInput.value &&
          refInput.value !== (item.referencia_admin || "")
        ) {
    ...
    if (nuevoEstado === "completado" && !pedidoActual.venta_registrada) {
      try {
        console.log(
          `📦 Registrando ventas para pedido #${pedidoActual.numero_pedido}...`
        );
        for (const item of pedidoActual.pedido_items || []) {
          console.log(
            `🔹 Item: ${item.productos?.nombre || "Producto"} | Vendedor: ${
              perfil?.cliente_uuid || "NULL!"
            }`
          );
          ...
          if (rpcError) {
            ...
            throw new Error(
              rpcError.message || "Error desconocido en el servidor de ventas"
            );
          }
          if (data?.error) {
          ...
          if (item.productos?.entrega_automatica) {
            ...
            const { data: codeData, error: codeError } = await supabase.rpc(
              "asignar_codigo_pedido_item_rpc",
              {
                p_pedido_item_id: item.id,
              }
            );
            if (codeError)
              console.error("❌ Error al asignar código:", codeError);
            else if (codeData) console.log("✅ Código asignado:", codeData);
            else
              console.warn(
                "⚠️ No hay códigos disponibles en el baúl para este producto."
              );
          }
        }
        ...
        showAlert(
          "No se pudo completar el pedido porque falló el registro contable: " +
            err.message,
          "error"
        );
        ...
        try {
          ...
          if (prodId) {
            const { data: prodData } = await supabase
              .from("productos")
              .select("juego_id, juegos(cashback_activo)")
              .eq("id", prodId)
              .maybeSingle();
      ...
      if (
        config?.cashback_activo === "true" &&
        Number(config?.cashback_porcentaje) > 0 &&
        !pedidoActual.cashback_aplicado &&
        gameAllowsCashback
      ) {
        try {
          ...
          if (p > 0) {
            ...
            if (!isBs && pedidoActual.metodo_pago_id) {
              const { data: mData } = await supabase
                ...
                .eq("id", pedidoActual.metodo_pago_id)
                .maybeSingle();
              if (
                mData &&
            ...
            const { data: walletData } = await supabase
              .from("billeteras")
              .select("*")
              .eq("auth_user_id", pedidoActual.cliente_id)
              .maybeSingle();
    ...
    if (!error) {
      ...
      setPedidos(
        pedidos.map((p) => (p.id === pedidoId ? { ...p, ...updateData } : p))
      );
    ...
    if (selectedPedido?.id === pedidoId) {
      ...
      if (updatedPedido) {
        setSelectedPedido({
          ...
          atendido_por: selectedPedido.atendido_por,
        });
      }
    }
  };

  // processTiendaGiftVenOrder is imported from apiProcessor
  ...
  const handleReembolso = async (pedido) => {
    ...
    showAlert(
      `¿A cuál billetera deseas reembolsar este pedido?\n\n💵 USD: $${Number(
        pedido.total_usd
      ).toFixed(2)}\n🏦 Bs: Bs ${Number(pedido.total_bs).toLocaleString(
        "es-VE"
      )}`,
      "confirm",
      async () => {
  ...
  const handleReembolsoSelect = (pedido) => {
    ...
    if (isBsUsed || isUsdUsed) {
      const match = pedido.referencia_pago.match(
        /billetera\s+(bs|usd):\s*\$?\s*([0-9.,]+)/i
      );
      if (match && match[2]) {
    ...
    setReembolsoMonto(
      prefillMonto || (isUsdUsed ? pedido.total_usd : pedido.total_bs)
    );
  ...
  const ejecutarReembolso = async (pedido, moneda, monto, cambiarEstado) => {
    ...
    const { data, error } = await supabase.rpc("reembolsar_pedido_rpc", {
      ...
      p_admin_id: user.id,
      p_notas: `Reembolso administrativo ${
        cambiarEstado ? "y cancelación" : "parcial"
      } por pedido #${pedido.numero_pedido} (${
        moneda === "bs" ? "Bolívares" : "USD"
      })`,
      p_moneda: moneda,
    ...
    if (error) {
      ...
      playCashRegisterSound();
      const monedaLabel =
        moneda === "bs" ? formatBs(monto) : `$${Number(monto).toFixed(2)}`;
      showAlert(
        `✅ Reembolso exitoso. ${monedaLabel} acreditados a la billetera ${
          moneda === "bs" ? "Bolívares" : "USD"
        } del cliente.`,
        "success"
      );
        ...
        prev.map((p) => {
          if (p.id === pedido.id) {
            ...
            if (cambiarEstado && updatedPedido.pedido_items) {
              updatedPedido.pedido_items = updatedPedido.pedido_items.map(
                (item) => ({
                  ...item,
        ...
        >
          <div
            style={{
              ...
              right: 0,
              height: "4px",
              background: "linear-gradient(to right, #e040fb, #8b5cf6)",
            }}
          ></div>
          ...
          <div style={{ textAlign: "center", marginBottom: "20px" }}>
            ...
            <h2
              style={{ fontSize: "20px", color: "#fff", marginBottom: "4px" }}
            >
              Reembolso de Billetera
            </h2>
            <p style={{ color: "var(--text-muted)", fontSize: "13px" }}>
              Pedido #{reembolsoPedido.numero_pedido} | Ref:{" "}
              {reembolsoPedido.referencia_pago}
            </p>
          </div>
          ...
          >
            ...
            <div>
              ...
              >
                Monto a acreditar ({reembolsoMoneda.toUpperCase()}):
              </label>
              <input
                type="number"
                step="0.01"
            ...
            >
              ...
              <span style={{ fontSize: "13px", color: "#fff" }}>
                Marcar el pedido como{" "}
                <span style={{ color: "#e040fb", fontWeight: 700 }}>
                  REEMBOLSADO
                </span>{" "}
                al cliente. <br />
                <span style={{ fontSize: "11px", color: "var(--text-muted)" }}>
                  (Desmárcalo si solo quieres devolver saldo pero mantener el
                  pedido como Fallido)
                </span>
              </span>
            </label>
    ...
    if (esValido) {
      ...
      try {
        ...
        const { data: pedidoFull } = await supabase
          .from("pedidos")
          .select(
            "*, pedido_items(*, productos(*, juegos(procesamiento_automatico_api, imagen_pedido_completado_url)))"
          )
          .eq("id", pedido.id)
          .single();
        ...
        if (data) {
          ...
          setPedidos((prev) =>
            prev.map((p) => (p.id === data.id ? pedFinal : p))
          );
          playCashRegisterSound();

          if (modoApi) {
            // Intentar baúl primero
            const baul = await processAutoDeliveryOrder(data.id).catch(
              () => false
            );
            if (baul) {
              refreshPedidoData(data.id);
              showAlert(
                "✅ Pago verificado y recarga procesada automáticamente desde el baúl.",
                "success"
              );
              return;
            }
            ...
            showAlert(
              "✅ Pago verificado. El pedido está listo para ser tomado y procesado manualmente.",
              "success"
            );
          }
        }
  ...
  const handleTomarPedido = async (pedido) => {
    ...
    const activosMios = pedidos.filter(
      (p) =>
  ...
  const handleLiberarPedido = async (pedido) => {
    showAlert(
      "¿Seguro que deseas liberar este pedido?",
      "confirm",
      async () => {
        const { error } = await supabase
          .from("pedidos")
        ...
        if (!error) {
          const pedActualizado = {
            ...pedido,
            atendido_por_id: null,
            estado: "pendiente",
          };
          setPedidos((prev) =>
            prev.map((p) => (p.id === pedido.id ? pedActualizado : p))
          );
  ...
  const handleAsignarAdmin = async () => {
    ...
    const upd = {
      ...selectedPedido,
      atendido_por_id: adminSeleccionado.auth_user_id,
      atendido_por: adminSeleccionado,
      estado: "procesando",
    };
    setPedidos((prev) =>
      prev.map((p) => (p.id === selectedPedido.id ? upd : p))
    );
    setSelectedPedido(upd);
    setShowAsignarAdminModal(false);

    showAlert(
      `✅ Pedido asignado a ${adminSeleccionado.nombres} ${
        adminSeleccionado.apellidos || ""
      } correctamente.`,
      "success"
    );
  };
  ...
  const updateEstadoConAdmin = async (pedidoId, nuevoEstado, adminTarget) => {
    ...
    if (fetchError || !pedidoActual) {
      showAlert(
        "No se pudo obtener el pedido: " + (fetchError?.message || ""),
        "error"
      );
      return;
    }
    ...
    if (!vendedorId) {
      showAlert(
        "El administrador seleccionado no tiene un perfil válido para acreditar el saldo.",
        "error"
      );
      return;
    }
        ...
        for (const item of pedidoActual.pedido_items || []) {
          const { data, error: rpcError } = await supabase.rpc(
            "registrar_venta_rpc",
            {
              p_producto_id: item.producto_id,
              p_cantidad: item.cantidad,
              p_notas: `Pedido #${pedidoActual.numero_pedido} (Atribuido a ${
                adminTarget.nombres || ""
              } por Super Admin)`,
              p_cliente_id: pedidoActual.cliente_id,
              p_metodo_pago_id: pedidoActual.metodo_pago_id,
          ...
          if (item.productos?.entrega_automatica) {
            await supabase.rpc("asignar_codigo_pedido_item_rpc", {
              p_pedido_item_id: item.id,
            });
          }
        }
    ...
    showAlert(
      `✅ Pedido marcado como COMPLETADO y acreditado a ${
        adminTarget.nombres || ""
      }.`,
      "success"
    );
    ...
    showAlert(
      ...
      async () => {
        try {
          ...
          if (updateError)
            throw new Error(
              "Error al revertir el pedido: " + updateError.message
            );
          ...
          setSelectedPedido(upd);
          showAlert(
            `✅ Pedido #${pedido.numero_pedido} reversado. El saldo del operario fue ajustado.`,
            "success"
          );
        } catch (err) {
        ...
        >
          <div style={{ position: "relative", marginBottom: "20px" }}>
            <div
              style={{
                height: "4px",
                background: "linear-gradient(90deg,#8b5cf6,#00d2ff)",
            ...
            >
              📤 Asignar Pedido a Administrador
            </h2>
            <p
              style={{
                fontSize: "13px",
            ...
            >
              Selecciona el operario que se hará cargo de este pedido. El estado
              cambiará a{" "}
              <strong style={{ color: "#8b5cf6" }}>PROCESANDO</strong>.
            </p>
          </div>
          {loadingAdmins ? (
              ...
              {adminsList.map((a) => (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: "12px",
                    border:
                      adminSeleccionado?.auth_user_id === a.auth_user_id
                        ? "2px solid #8b5cf6"
          ...
          <div style={{ display: "flex", gap: "10px" }}>
            <button
              ...
              style={{
                ...
                borderRadius: "12px",
                border: "none",
                backgroundColor: adminSeleccionado
                  ? "#8b5cf6"
                  : "rgba(255,255,255,0.1)",
                color: "#fff",
                fontWeight: 700,
                fontSize: "14px",
                cursor: adminSeleccionado ? "pointer" : "not-allowed",
                transition: "all 0.2s",
              }}
            >
              📤 Asignar Pedido
      ...
      >
        ...
        >
          <div style={{ position: "relative", marginBottom: "20px" }}>
            ...
            >
              Selecciona el operario que processó este pedido. El monto{" "}
              <strong style={{ color: "#22c55e" }}>
                se acreditará a su billetera operativa
              </strong>
              .
            </p>
            <div
            ...
            >
              ⚠️ Esto marcará el pedido como <strong>COMPLETADO</strong> y
              registrará la venta bajo el operario elegido. Esta acción es
              irreversible.
            </div>
          </div>
          {loadingAdmins ? (
                ...
                >
                  <div
                    style={{
                      ...
                      height: "38px",
                      borderRadius: "50%",
                      background: "linear-gradient(135deg,#22c55e,#00d2ff)",
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                      fontSize: "17px",
          ...
          <div style={{ display: "flex", gap: "10px" }}>
            <button
              onClick={() => {
                ...
                showAlert(
                  `¿Confirmar que el pedido fue procesado por ${adminSeleccionado.nombres}? El monto se acreditará a su saldo operativo.`,
                  "confirm",
                  () =>
                    updateEstadoConAdmin(
                      selectedPedido.id,
                      "completado",
                      adminSeleccionado
                    )
      ...
      >
        ...
        >
          ...
          <div style={{ textAlign: "center", marginBottom: "24px" }}>
            ...
            <h2
              style={{ fontSize: "22px", color: "#fff", marginBottom: "4px" }}
            >
              {maskSensitive(
                modalClient.nombres + " " + (modalClient.apellidos || "")
              )}
            </h2>
            <p
              style={{
            ...
            >
              @{maskSensitive(modalClient.nickname || "Usuario")}
            </p>
          </div>
          ...
          <div style={{ display: "grid", gap: "16px" }}>
            ...
            >
              ...
              >
                {modalClient.whatsapp
                  ? maskSensitive(modalClient.whatsapp, "phone")
                  : "No suministrado"}
                {modalClient.whatsapp && !isEmpleado && (
                  <a
                    href={`https://wa.me/${modalClient.whatsapp.replace(
                      /\D/g,
                      ""
                    )}`}
                    target="_blank"
            ...
            >
              ...
              <div style={{ color: "#fff", fontWeight: 600, fontSize: "16px" }}>
                {modalClient.nickname || "Sin nickname"}
              </div>
            </div>
            ...
            >
              ...
              <div style={{ color: "#fff", fontWeight: 600, fontSize: "16px" }}>
                {formatFecha(
                  modalClient.created_at || modalClient.fecha_registro
                )}
              </div>
            </div>
          </div>
  ...
  const updateItemEstado = async (itemId, nuevoEstado, adminMsg = null) => {
    ...
    if (error) {
      showAlert(
        "Error al actualizar estado del paquete: " + error.message,
        "error"
      );
      return;
    }
    ...
    setSelectedPedido((prev) => {
      ...
      const newItems = prev.pedido_items.map((i) =>
    ...
    setPedidos((prev) =>
      prev.map((p) => {
        if (p.id === selectedPedido?.id) {
          const newItems = p.pedido_items?.map((i) =>
            i.id === itemId ? { ...i, ...updateData } : i
          );
  ...
  const updateItemReference = async (itemId, referencia) => {
    const { error } = await supabase
      .from("pedido_items")
      .update({ referencia_admin: referencia })
      .eq("id", itemId);
    if (error) {
    ...
    setSelectedPedido((prev) => {
      if (!prev || !prev.pedido_items) return prev;
      const newItems = prev.pedido_items.map((i) =>
        i.id === itemId ? { ...i, referencia_admin: referencia } : i
      );
      return { ...prev, pedido_items: newItems };
    });
  ...
  const updateItemCodigo = async (itemId, codigo) => {
    const { error } = await supabase
      .from("pedido_items")
      .update({ codigo_entregado: codigo })
      .eq("id", itemId);
    if (error) {
    ...
    setSelectedPedido((prev) => {
      if (!prev || !prev.pedido_items) return prev;
      const newItems = prev.pedido_items.map((i) =>
        i.id === itemId ? { ...i, codigo_entregado: codigo } : i
      );
      return { ...prev, pedido_items: newItems };
    });
  ...
  const handleConfirmRechazo = () => {
    if (!motivoRechazo.trim()) {
      showAlert(
        "Debes escribir un motivo por el cual no se pudo recargar este paquete.",
        "error"
      );
      return;
    }
  ...
  const confirmarCancelacionPagoNoEncontrado = async () => {
    ...
    try {
      ...
      const isUsdTotal =
        refBaja.includes("pago_billetera_usd_total") ||
        refBaja.startsWith("pago_billetera_usd");
      const isBsTotal =
        refBaja.includes("pago_billetera_bs_total") ||
        refBaja.startsWith("pago_billetera_bs");
      const needsRefund =
        refBaja.includes("billetera bs") ||
        ...
        if (isUsdTotal) {
          ...
          if (monto > 0) {
            const { data: refundResult, error: refundError } =
              await supabase.rpc("reembolsar_pedido_rpc", {
            ...
            if (!refundError && !refundResult?.error) {
              ...
              console.error(
                `Error en reembolso automático (usd):`,
          ...
          if (monto > 0) {
            ...
            if (!refundError && !refundResult?.error) {
              ...
              console.error(
                `Error en reembolso automático (bs):`,
                refundError || refundResult?.error
              );
            }
          }
          ...
          while (
            ...
            if (monto > 0) {
              ...
              if (!refundError && !refundResult?.error) {
                reembolsosRealizados.push(
                  moneda === "bs" ? formatBs(monto) : formatUSD(monto)
                );
              } else {
                console.error(
                  `Error en reembolso automático (${moneda}):`,
                  refundError || refundResult?.error
                );
              }
            }
      ...
      if (reembolsosRealizados.length > 0) {
        msgFinal += `\n\nPD: El saldo de tu billetera utilizado en este pedido (${reembolsosRealizados.join(
          " y "
        )}) ha sido reintegrado exitosamente a tu cuenta.`;
      ...
      if (selectedPedido.cliente?.id) {
        ...
        if (chatError)
          console.error("Error enviando mensaje al chat:", chatError);
      }
      ...
      const pedFinal = {
        ...selectedPedido,
        ...updateData,
        reembolso_billetera: reembolsosRealizados.length > 0,
      };
      setPedidos((prev) =>
        prev.map((p) => (p.id === selectedPedido.id ? pedFinal : p))
      );
      setSelectedPedido(pedFinal);

      const alertMsg =
        reembolsosRealizados.length > 0
          ? `✅ Pedido cancelado. Se reintegraron ${reembolsosRealizados.join(
              " y "
  ...
  const handleUploadImage = async (file) => {
    ...
    if (!error) {
      const {
        data: { publicUrl },
      } = supabase.storage.from("pedidos-adjuntos").getPublicUrl(fileName);
  ...
  const renderReferenciaConMonto = (ref, totalBs, totalUsd) => {
    ...
    if (refNum && (remainingBs > 0 || remainingUsd > 0)) {
      const montoRefLabel =
        remainingBs > 0 ? formatBs(remainingBs) : formatUSD(remainingUsd);
      return (
        <div
          style={{
            display: "flex",
          ...
          <div className="page-header mb-8 pedidos-header-responsive">
            <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
              ...
              <h1
                ...
                style={{ margin: 0, fontSize: "16px", fontWeight: 800 }}
              >
              ...
              <span style={{ fontSize: "10px", color: "var(--text-muted)" }}>
                {formatFecha(selectedPedido.created_at)}
              </span>
            </div>
            ...
            {canManage && (
              <div
                className="pedidos-header-actions"
                ...
                {selectedPedido.estado === "cancelado" ? (
                  <>
                    ...
                    <button
                      ...
                      onClick={() =>
                        showAlert(
                          "¿Deseas marcar este pedido como PENDIENTE para que pueda ser gestionado nuevamente?",
                          "confirm",
                          async () => {
                            const { error } = await supabase
                              .from("pedidos")
                            ...
                            if (!error) {
                              ...
                              setPedidos((prev) =>
                                prev.map((p) =>
                                  p.id === selectedPedido.id ? upd : p
                                )
                              );
                              setSelectedPedido(upd);
                              showAlert(
                                "✅ Pedido restablecido a PENDIENTE.",
                                "success"
                              );
                            } else {
                              showAlert(
                    ...
                    <button
                      ...
                      onClick={() =>
                        showAlert(
                          "¿Confirmar que este pedido fue COMPLETADO?",
                          "confirm",
                  ...
                  <button
                    ...
                    style={{
                      ...
                      fontSize: "12px",
                      backgroundColor:
                        selectedPedido.pago_verificado === true
                          ? "#8b5cf6"
                          : "var(--text-muted)",
                      borderColor:
                        selectedPedido.pago_verificado === true
                          ...
                          : "var(--text-muted)",
                      opacity:
                        selectedPedido.pago_verificado === true ? 1 : 0.5,
                      cursor:
                        selectedPedido.pago_verificado === true
                          ? "pointer"
                          : "not-allowed",
                    ...
                    title={
                      selectedPedido.pago_verificado === true
                        ? "Tomar pedido para procesar"
                        : "Verifica el pago primero"
                    }
                  >
                    📥 Tomar pedido
                  </button>
                  ...
                  <>
                    {selectedPedido.estado !== "pendiente" && (
                      <button
                        ...
                        onClick={() =>
                          updateEstado(selectedPedido.id, "pendiente")
                        }
                      >
                        ⏳ Pendiente
                      </button>
                    ...
                    {selectedPedido.estado !== "pago_no_encontrado" && (
                      <button
                        ...
                        onClick={() =>
                          updateEstado(selectedPedido.id, "pago_no_encontrado")
                        }
                      >
                        🔍 Pago No Encontrado
                      </button>
                    ...
                    {selectedPedido.estado !== "pago_duplicado" && (
                      <button
                        ...
                        onClick={() =>
                          updateEstado(selectedPedido.id, "pago_duplicado")
                        }
                      >
                        ⚠️ Pago Duplicado
                      </button>
                    ...
                    {/* Botón de reembolso */}
                    {!["completado", "reembolsado"].includes(
            ...
            <div className="card" style={{ padding: "12px" }}>
              ...
              >
                ...
                {selectedPedido.cliente ? (
                  <div
                    ...
                    onClick={() => {
                      setModalClient(selectedPedido.cliente);
                  ...
                  >
                    <div style={{ textAlign: "right" }}>
                      ...
                      >
                        {maskSensitive(
                          selectedPedido.cliente.nombres +
                            " " +
                            (selectedPedido.cliente.apellidos || "")
                        )}
                      </div>
                      <div
                        style={{
                      ...
                      >
                        {maskSensitive(
                          selectedPedido.cliente.whatsapp,
                          "phone"
                        )}
                      </div>
                    </div>
              ...
              <div style={{ display: "grid", gap: "8px" }}>
                ...
                <div className="summary-row" style={{ display: "block" }}>
                  ...
                  >
                    ...
                    >
                      {renderReferenciaConMonto(
                        selectedPedido.referencia_pago,
                        selectedPedido.total_bs,
                        selectedPedido.total_usd
                      )}
                    </span>
                  </div>
                  <div
                    ...
                    >
                      <span
                        style={{
                          ...
                          color:
                            selectedPedido.pago_verificado === true
                              ? "var(--accent-success)"
                              : selectedPedido.pago_verificado === false
                              ? "var(--accent-error)"
                              : "var(--text-muted)",
                        }}
                      >
                      ...
                      {canManage && (
                        <div style={{ display: "flex", gap: "4px" }}>
                          <button
                            onClick={() =>
                              handleVerificarPago(selectedPedido, true)
                            }
                            style={{
                              width: "24px",
                              height: "24px",
                          ...
                          <button
                            onClick={() =>
                              handleVerificarPago(selectedPedido, false)
                            }
                            style={{
                              width: "24px",
                              height: "24px",
                  ...
                  {canManage &&
                    ...
                    selectedPedido.estado !== "cancelado" && (
                      ...
                      >
                        ...
                        >
                          Si el pago no existe en el banco, puedes cancelar el
                          pedido con el siguiente mensaje predeterminado:
                        </p>

                        <textarea
                          value={cancelacionMensaje}
                          onChange={(e) =>
                            setCancelacionMensaje(e.target.value)
                          }
                          style={{
                            width: "100%",
                            height: "140px",
                ...
                <div className="summary-row notranslate" translate="no">
                  ...
                  <div style={{ textAlign: "right" }}>
                    ...
                    >
                      {formatBs(selectedPedido.total_bs)}
                    </div>
                    {canManage && (
                      <div
                        style={{ fontSize: "12px", color: "var(--text-muted)" }}
                      >
                        {formatUSD(selectedPedido.total_usd)}
                      </div>
                    )}
                  </div>
                </div>
                ...
                {selectedPedido.cupon_id && selectedPedido.cupones && (
                  <div
                    className="summary-row notranslate"
                    translate="no"
                    style={{
                  ...
                  >
                    ...
                    <div style={{ textAlign: "right" }}>
                      ...
                      >
                        -{selectedPedido.cupones.porcentaje_descuento}% de
                        descuento
                      </div>
                    </div>
                  </div>
                ...
                {selectedPedido.cashback_aplicado &&
                  (() => {
                    const p =
                      selectedPedido.cashback_porcentaje ||
                      config?.cashback_porcentaje ||
                      0;
                    const isBs =
                      selectedPedido.cashback_moneda === "bs" ||
                      (!selectedPedido.cashback_moneda &&
                        (selectedPedido.referencia_pago
                          ?.toLowerCase()
                          .includes("bs") ||
                          selectedPedido.referencia_pago
                          ...
                          selectedPedido.referencia_pago
                            ...
                            .includes("movil")));
                    const monto =
                      selectedPedido.cashback_monto ||
                      (isBs
                        ? Number(selectedPedido.total_bs) * (p / 100)
                        : Number(selectedPedido.total_usd) * (p / 100));
                    return (
                      <div
                        translate="no"
                        className="notranslate"
                ...
                {selectedPedido.comprobante_url ? (
                  <div style={{ marginTop: "4px" }}>
                    ...
                    <div
                      ...
                      onClick={() =>
                        window.open(selectedPedido.comprobante_url, "_blank")
                      }
                    >
                      <img
                        loading="lazy"
                      ...
                      >
                        {esElOperador(selectedPedido)
                          ? perfil.nickname ||
                            ...
                            perfil.usuario ||
                            "Tú"
                          : selectedPedido.atendido_por?.nickname ||
                            selectedPedido.atendido_por?.nombres ||
                            "Otro Admin"}
                      </span>
              ...
              {selectedPedido.fecha_respuesta && (
                ...
                >
                  ...
                  >
                    {formatFecha(selectedPedido.fecha_respuesta)}
                  </span>
                </div>
              )}
              ...
              {(() => {
                const images = selectedPedido.imagenes_adjuntas
                  ? JSON.parse(selectedPedido.imagenes_adjuntas)
                  : [];
              ...
              {canManage && (
                ...
                >
                  <div
                    className="pedidos-gestion-grid"
                    style={{
                      display: "grid",
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setSelectedPedido((prev) => ({
                            ...prev,
                            referencia_recarga: e.target.value,
                          }))
                        }
                        onBlur={(e) =>
                          updatePedidoField(
                  ...
                  >
                    ...
                    <input
                      ...
                      onFocus={(e) => {
                        e.target.placeholder = "Presiona Ctrl+V ahora...";
                        e.target.style.borderColor = "var(--accent-primary)";
                      }}
                      onBlur={(e) => {
                        e.target.placeholder =
                          "Click aquí y Ctrl+V para pegar imagen";
                ...
                {(selectedPedido.pedido_items || []).map((item, idx) => (
                  ...
                  >
                        ...
                        <div>
                          ...
                          >
                            ...
                            {item.estado_proveedor && (
                                ...
                                style={{
                                  backgroundColor:
                                    ...
                                    item.estado_proveedor === "fallido"
                                      ...
                                      : item.estado_proveedor ===
                                          "completado" ||
                                        item.estado_proveedor === "completed" ||
                                        item.estado_proveedor === "aprobado"
                                      ? "rgba(34, 197, 94, 0.1)"
                                      : "rgba(251, 191, 36, 0.1)",
                                  color:
                                    item.estado_proveedor === "error" ||
                              ...
                              >
                                API: {item.estado_proveedor.toUpperCase()}{" "}
                                {item.estado_proveedor === "procesando" ||
                                item.estado_proveedor === "processing"
                                  ? "⏳"
                                  : item.estado_proveedor === "completado" ||
                                    item.estado_proveedor === "completed" ||
                                    item.estado_proveedor === "aprobado"
                                  ? "✅"
                                  : "❌"}
                            ...
                            >
                              <button
                                ...
                                title={
                                  item.estado === "completado"
                                    ? "Desmarcar completo"
                                    : "Marcar como recargado/completado"
                                }
                              >
                                ✓
                              </button>
                              <button
                                onClick={() => {
                                  setRechazandoItem(
                                    item.id === rechazandoItem ? null : item.id
                                  );
                                  setMotivoRechazo(item.notas_admin || "");
                                }}
                                style={{
                                  width: "32px",
                                  height: "32px",
                    ...
                    {canManage && selectedPedido.estado !== "completado" ? (
                      <div style={{ marginBottom: "8px" }}>
                        <input
                          ...
                          onBlur={(e) => {
                            if (
                              e.target.value !== (item.referencia_admin || "")
                            ) {
                        ...
                        <div style={{ marginTop: "8px" }}>
                          <input
                            ...
                            onBlur={(e) => {
                              if (
                                e.target.value !== (item.codigo_entregado || "")
                              ) {
                    ...
                    >
                      ...
                      >
                        {item.metodo_recarga === "solo_correo" ? (
                          ...
                          >
                            ...
                            >
                              👤 {item.account_user || item.account_email}
                            </div>
                          </div>
                          ...
                          >
                            ...
                            >
                              Nosotros proporcionaremos la cuenta
                            </div>
                          </div>
                      ...
                      {item.estado === "completado" &&
                        item.productos?.juegos
                          ?.imagen_pedido_completado_url && (
                          <div
                            style={{
                              width: "80px",
                              height: "80px",
                    ...
                    {item.codigo_entregado && (
                      ...
                      >
                        ...
                        >
                          ...
                          {(item.productos?.juegos?.url_canje ||
                            (Array.isArray(item.productos) &&
                              item.productos[0]?.juegos?.url_canje)) && (
                            <a
                              href={
                                item.productos?.juegos?.url_canje ||
                                item.productos[0]?.juegos?.url_canje
                    ...
                    {rechazandoItem === item.id && canManage && (
                      ...
                      >
                        ...
                        >
                          <button
                            onClick={() => setRechazandoItem(null)}
                            style={{
                              padding: "6px 16px",
                              borderRadius: "8px",
                              border: "1px solid var(--border-color)",
                    ...
                    {canManage &&
                      (() => {
                        let opcionesJuego =
                          item.productos?.juegos?.opciones_recarga || [];
                        if (typeof opcionesJuego === "string") {
                          try {
                            opcionesJuego = JSON.parse(opcionesJuego);
                          } catch (e) {
                            opcionesJuego = [];
                          }
                        }
                        ...
                        let opcionesProd =
                          item.productos?.opciones_recarga || [];
                        if (typeof opcionesProd === "string") {
                          try {
                            opcionesProd = JSON.parse(opcionesProd);
                          } catch (e) {
                            opcionesProd = [];
                          }
                        }
                    ...
                    {(item.productos?.juegos?.tutorial_banner_img ||
                      ...
                      <div
                        ...
                        style={{
                          position: "relative",
                          cursor: item.productos?.juegos?.tutorial_video_url
                            ? "pointer"
                            : "default",
                          borderRadius: "16px",
                          overflow: "hidden",
                      ...
                      >
                        {item.productos.juegos.tutorial_banner_img ? (
                          <img
                            loading="lazy"
                            decoding="async"
                            src={item.productos.juegos.tutorial_banner_img}
                            alt="Tutorial"
                          ...
                          >
                            ...
                            <div>
                              ...
                              >
                                {item.productos.juegos.tutorial_banner_texto ||
                                  `¿Cómo recargar?`}
                              </h4>
                              <p
                                style={{
        ...
        {viewingOpcion && (
          ...
          >
            ...
            >
              ...
              >
                {viewingOpcion.mensaje ||
                  "No hay instrucciones para esta opción."}
              </div>
  ...
  if (config?.admin_interface === "lootadmin" && canManage && !embedded) {
    const LootAdminComponent = React.lazy(() =>
      import("./lootadmin/PedidosLootAdmin")
    );
    return (
      <React.Suspense
        fallback={
    ...
    <>
      <div
        style={{
          ...
          display: "flex",
          flexDirection: "column",
          padding: embedded ? "0" : "0 32px 32px",
        }}
      >
        <style>{`
                ...
                <>
                  ...
                  >
                    {FILTROS.map((f) => {
                      ...
                      const style =
                        f.key !== "todos"
                          ? getEstadoStyle(f.key)
                          : { bg: "transparent", color: "var(--text-primary)" };
                  ...
                  <table className="orders-table table-cards-mobile">
                    <thead>
                      <tr>
                        ...
                        <th className="desktop-only" style={{ width: "120px" }}>
                          Fecha
                        </th>
                        <th
                          style={{
                        ...
                        <th className="desktop-only" style={{ width: "90px" }}>
                          Ref
                        </th>
                      ...
                      {currentPedidos.map((pedido) => {
                        ...
                        const juegos = [
                          ...new Set(
                            items.map(
                              (i) =>
                                i.juego_nombre || i.productos?.juegos?.nombre
                            )
                          ),
                        ];
                        const paquetes = items.map(
                          (i) =>
                            `${
                              i.producto_nombre ||
                              i.productos?.nombre ||
                        ...
                        return (
                          <tr
                            ...
                            className={`pedido-row-modern ${
                              esElOperador(pedido) ? "assigned-to-me" : ""
                            }`}
                          >
                            <td
                              data-label="ID"
                                ...
                                {pedido.atendido_por && (
                                  <span
                                    style={{
                                      ...
                                      padding: "2px 6px",
                                      borderRadius: "4px",
                                      backgroundColor: esElOperador(pedido)
                                        ? "rgba(139, 92, 246, 0.2)"
                                        : "rgba(255, 255, 255, 0.1)",
                                      color: esElOperador(pedido)
                                        ? "#d8b4fe"
                                        : "#a1a1aa",
                                      border: `1px solid ${
                                        esElOperador(pedido)
                                  ...
                                  >
                                    ...
                                    {esElOperador(pedido)
                                      ? "Asignado a ti"
                                      : pedido.atendido_por.nombres || "Admin"}
                                  </span>
                                )}
                              </div>
                            </td>
                              ...
                              {pedido.cliente
                                ? maskSensitive(
                                    `${pedido.cliente.nombres} ${
                                      pedido.cliente.apellidos?.toLowerCase() ===
                                      "pendiente"
                                        ? ""
                            ...
                            <td data-label="Pago">
                              {(() => {
                                ...
                                if (pedido.pago_verificado === true) {
                                  ...
                                  pedido.estado === "pago_no_encontrado"
                                ) {
                                  text = "NO ENCONTRADO";
                                  color = "#ef4444";
