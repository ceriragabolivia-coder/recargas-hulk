                      ...
                      <div>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '4px', flexWrap: 'wrap' }}>
                          ...
                          {item.estado_proveedor && (
                            <span style={{ backgroundColor: item.estado_proveedor === 'error' || item.estado_proveedor === 'fallido' ? 'rgba(239, 68, 68, 0.1)' : item.estado_proveedor === 'completado' || item.estado_proveedor === 'completed' || item.estado_proveedor === 'aprobado' ? 'rgba(34, 197, 94, 0.1)' : 'rgba(251, 191, 36, 0.1)', color: item.estado_proveedor === 'error' || item.estado_proveedor === 'fallido' ? 'var(--accent-error)' : item.estado_proveedor === 'completado' || item.estado_proveedor === 'completed' || item.estado_proveedor === 'aprobado' ? '#22c55e' : '#fbbf24', padding: '4px 8px', borderRadius: '6px', fontSize: '11px', fontWeight: 800, display: 'inline-flex', alignItems: 'center', gap: '4px', border: `1px solid ${item.estado_proveedor === 'error' || item.estado_proveedor === 'fallido' ? 'rgba(239, 68, 68, 0.3)' : item.estado_proveedor === 'completado' || item.estado_proveedor === 'completed' || item.estado_proveedor === 'aprobado' ? 'rgba(34, 197, 94, 0.3)' : 'rgba(251, 191, 36, 0.3)'}` }}>
                              API: {item.estado_proveedor.toUpperCase()} {item.estado_proveedor === 'procesando' || item.estado_proveedor === 'processing' ? '⏳' : item.estado_proveedor === 'completado' || item.estado_proveedor === 'completed' || item.estado_proveedor === 'aprobado' ? '✅' : '❌'}
                            </span>
                          )}
                        </div>
