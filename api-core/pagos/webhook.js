...
async function procesarPedidoConPinCentral(pedidoId, apiKey, apiSecret) {
  ...
  for (const item of pedidoActual.pedido_items) {
    ...
    if (
      ...
      anySent = true;
      try {
        console.log(`🚀 [Webhook] Enviando item ${item.id} a PinCentral...`);
        ...
        if (data && (data.status === 'created' || data.status === 'processing' || data.status === 'completed' || data.status === 'authorized')) {
          ...
          if (!isRecharge && respEstado === 'authorized') {
            console.log(`🚀 [Webhook] Capturando PIN para item ${item.id}...`);
        ...
        console.error(`❌ [Webhook] Error en item ${item.id} con PinCentral:`, e.message);
        allCompleted = false;
        await supabase.rpc("webhook_update_pedido_item", {
          p_item_id: item.id,
  ...
  for (const item of pedidoActual.pedido_items) {
    ...
    if (
      ...
      try {
          ...
          await supabase.rpc("webhook_update_pedido_item", {
            ...
            p_mensaje_proveedor:
              Array.isArray(data.codigos) && data.codigos.length > 0
                ? data.codigos.join("\n")
                : data.codigos && typeof data.codigos === "string"
                ? data.codigos
                : data.mensaje || "",
            p_estado: isCompleted ? "completado" : "procesando",
          });
        ...
        console.error(
          `❌ [Webhook] Error en item ${item.id} con TiendaGiftVen:`,
          e.message
        );
        allCompleted = false;
        await supabase.rpc("webhook_update_pedido_item", {
      ...
      try {
        ...
        if (data.ok && data.order) {
          ...
          await supabase.rpc("webhook_update_pedido_item", {
            ...
            p_estado: isCompleted ? "completado" : "procesando",
            p_codigo_entregado: extractedPin || null,
          });
        } else {
        ...
        console.error(
          `❌ [Webhook] Error en item ${item.id} con FazerCards:`,
          e.message
        );
        allCompleted = false;
        await supabase.rpc("webhook_update_pedido_item", {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!referencia || !monto) {
      return res
        .status(400)
        .json({ error: "Faltan campos requeridos (referencia o monto)" });
    }

    // Workaround: The Android app sometimes sends the wrong amount due to a caching bug.
    // We try to extract the correct amount from the original SMS text.
    if (texto_original) {
      const match = texto_original.match(/Bs\.?\s*([\d.,]+)/i);
      if (match) {
        const cleanMonto = match[1].replace(/\./g, "").replace(",", ".");
    ...
    const isApkEnabled = apkConfig
      ? apkConfig.valor_texto === "true" || apkConfig.valor === true
      : true;

    if (!isApkEnabled) {
      console.log(
        "🚫 Recepción de Pagos APK deshabilitada. Referencia ignorada:",
        referencia
      );
    ...
    if (pedido && !pedidoError) {
      ...
      if (pedido.estado === "pendiente") {
        ...
        if (
          Math.abs(montoRecibido - montoEsperado) <= 0.05 ||
          ...
          console.log(
            `✅ Pago verificado automáticamente para pedido #${pedido.id}`
          );

          console.log(`📦 Intentando auto-despacho desde baúl local para pedido #${pedido.id}`);
          const { data: rpcData, error: rpcError } = await supabase.rpc(
            "procesar_pedido_automatico_rpc",
            { p_pedido_id: pedido.id }
          );

          if (!rpcError && rpcData?.success && rpcData?.completado) {
            auto_despachado = true;
            console.log(`🎉 Pedido #${pedido.id} completado automáticamente desde el baúl local`);
          } else {
          ...
          if (tieneApiItems) {
            ...
            const firstApiItem = pedidoConItems.pedido_items.find((i) => {
              const p = Array.isArray(i.productos)
                ? i.productos[0]
                : i.productos;
              const j = Array.isArray(p?.juegos) ? p.juegos[0] : p?.juegos;
              return p?.proveedor_api_id && j?.procesamiento_automatico_api;
            ...
            if (firstApiItem) {
              ...
              const j = Array.isArray(p?.juegos) ? p.juegos[0] : p?.juegos;
              providerName =
                p?.api_provider || j?.api_provider || "tiendagiftven";
            }

            console.log(
              `⚡ Pedido tiene items con proveedor_api_id. Proveedor: ${providerName}. Llamando API automáticamente...`
            );

            // Obtener API key de configuración
            const configRows = await supabase
              .from("configuracion")
            ...
            if (providerName === "pincentral") {
              apiKey = configRows.data?.find(r => r.clave === 'pincentral_api_key')?.valor_texto;
              apiSecret = configRows.data?.find(r => r.clave === 'pincentral_api_secret')?.valor_texto;
            } else {
              const configKey = providerName === "fazercards" ? "fazercards_api_key" : "tiendagiftven_api_key";
              const row = configRows.data?.find(r => r.clave === configKey);
            ...
            if (apiKey && (providerName !== "pincentral" || apiSecret)) {
              ...
              console.log(
                `🔑 Obteniendo items del pedido para procesar con API...`
              );
              ...
              if (providerName === "fazercards") {
                const resProvider = await procesarPedidoConFazerCards(
                  pedido.id,
                  apiKey
                );
                ...
                const resProvider = await procesarPedidoConPinCentral(
                  pedido.id,
                  apiKey,
                  apiSecret
                );
                ...
                const resProvider = await procesarPedidoConApi(
                  pedido.id,
                  apiKey
                );
                anySent = resProvider.anySent;
              ...
              if (anySent && allCompleted) {
                ...
                console.log(
                  `⏳ Pedido #${pedido.id} en procesamiento. Algunos items no completados aún.`
                );
                auto_despachado = true;
              } else {
                console.warn(
                  `⚠️ Pedido #${pedido.id}: no se pudo enviar a la API.`
                );
              }
            } else {
              console.warn(
                `⚠️ No hay ${configKey} en configuración. Pago verificado pero no procesado vía API.`
              );
            }
          } else {
            // Sin auto-procesamiento API: solo queda con pago_verificado=true para proceso manual
            console.log(
              `📋 Pedido #${pedido.id}: pago verificado. Admin procesará manualmente.`
            );
          }
          }
        } else {
          console.warn(
            `⚠️ Pedido #${pedido.id} monto no coincide. Esperado: ${montoEsperado}, Recibido: ${montoRecibido}`
          );
        }
      }
    }
        ...
        if (recarga.estado === "pendiente") {
          ...
          if (Math.abs(montoRecibido - montoEsperado) <= 2.0) {
            ...
            if (!processError && processData?.success) {
              ...
              console.log(
                `⚡ Recarga ${recarga.id} auto-aprobada vía Webhook APK`
              );
            } else {
              console.error(
                `❌ Error en auto-aprobación de recarga ${recarga.id}:`,
                processError || processData
              );
            }
          } else {
            console.warn(
              `⚠️ Recarga ${recarga.id} monto no coincide. Esperado: ${montoEsperado}, Recibido: ${montoRecibido}`
            );
          }
        }
      }
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        status:
          pedido_id || usuario_id || auto_despachado ? "usado" : "disponible",
        raw_data: payload,
      })
      .select()
      .single();
    ...
    if (error) {
      ...
      if (error.code === "23505") {
        return res
          .status(200)
          .json({ message: "Pago ya registrado anteriormente", data: null });
      }
      return res
        .status(500)
        .json({ error: "Error guardando el pago en la base de datos" });
    }
