...
export default async function handler(req, res) {
  ...
  try {
    ...
    const pedido_id = payload.pedido_id || payload.id_pedido || payload.id;
    const mensaje = payload.mensaje;
    const codigos = payload.codigos;
    let estado = (payload.estado || payload.status || '').toLowerCase();

    let final_merchant_ref = merchant_ref;
    if (!final_merchant_ref && pedido_id) {
      const { data: items } = await supabase.from('pedido_items').select('id').eq('proveedor_pedido_id', pedido_id).limit(1);
      if (items && items.length > 0) {
        final_merchant_ref = `ITEM-${items[0].id}`;
      }
    }

    if (!final_merchant_ref) {
      return res.status(400).json({ error: 'Missing merchant_ref or ref and could not map from pedido_id' });
    }
    ...
    const { data: rpcData, error: rpcError } = await supabase.rpc('procesar_webhook_tiendagiftven_rpc', {
      p_merchant_ref: final_merchant_ref,
