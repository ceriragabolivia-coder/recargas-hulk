...
export default async function handler(req, res) {
  ...
  if (cronSecretHeader && cronSecretHeader === expectedCronSecret) {
    ...
    if (error) debugInfo += `GetUserError: ${error.message} `;
    
    if (user?.user) {
      const userSupabase = createClient(supabaseUrl, supabaseKey, {
        global: { headers: { Authorization: `Bearer ${token}` } }
      });
      const { data: profile, error: profErr } = await userSupabase.from('perfiles').select('rol').eq('id', user.user.id).single();
      if (profErr) debugInfo += `ProfileError: ${profErr.message} `;
      
      if (profile?.rol === 'admin' || profile?.rol === 'administrador' || profile?.rol === 'negocio' || profile?.rol === 'superadmin') {
        isAdmin = true;
      } else {
        debugInfo += `RoleMismatch: ${profile?.rol} `;
      }
    }
  }

  if (!isAdmin && !isCron) {
    return res.status(401).json({ error: `Unauthorized. Debug: ${debugInfo} | URL: ${supabaseUrl}` });
  }
