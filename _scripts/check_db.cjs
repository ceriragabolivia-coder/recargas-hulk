...
conn.on('ready', () => {
  conn.exec('docker exec -i supabase-db psql -U postgres -d postgres -c "SELECT id, nombre, costo_base, precio_venta_fijo FROM public.productos LIMIT 10;"', (err, stream) => {
    let out = '';
