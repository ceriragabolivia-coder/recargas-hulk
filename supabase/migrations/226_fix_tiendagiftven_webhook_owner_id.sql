-- Migración 226: Corregir error owner_id inexistente en pedidos al procesar webhook de TiendaGiftVen
...
BEGIN
    ...
    SET 
        ...
        estado = CASE WHEN p_estado IN ('completado', 'aprobado', 'exitoso', 'procesado', 'realizado', 'completada', 'aprobada', 'exitosa', 'realizada') THEN 'completado'
            ...
            IF NOT COALESCE(v_order_record.venta_registrada, false) THEN
                FOR v_item_record IN (SELECT * FROM public.pedido_items WHERE pedido_id = v_pedido_interno_id) LOOP
                    PERFORM public.registrar_venta_rpc(
                        ...
                        'Pedido #' || COALESCE(v_order_record.numero_pedido::TEXT, v_pedido_interno_id::TEXT) || ' (API Webhook)',
                        ...
                        v_pedido_interno_id,
                        NULL -- <- CORREGIDO: NULL en lugar de v_order_record.owner_id ya que esa columna no existe
                    );
                END LOOP;
            END IF;
