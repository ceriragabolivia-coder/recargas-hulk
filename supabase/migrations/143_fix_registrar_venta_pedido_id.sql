-- Migration: 143_fix_registrar_venta_pedido_id.sql
-- Description: Cambiar el tipo de p_pedido_id de UUID a INT en registrar_venta_rpc para que coincida con el tipo de pedidos.id (INTEGER) y evitar errores de casteo al completar pedidos.

-- 1. Eliminar la función con la firma antigua (p_pedido_id como UUID)
DROP FUNCTION IF EXISTS public.registrar_venta_rpc(
    integer, 
    integer, 
    text, 
    uuid, 
...
-- 2. Crear la función con la firma corregida (p_pedido_id como INT)
