-- Migration 147: Cambiar el tipo de p_pedido_id de UUID a INT en reembolsar_pedido_rpc
-- Corrige el error de casteo cuando se intenta realizar un reembolso desde la interfaz web,
-- ya que la tabla public.pedidos utiliza una clave primaria INTEGER (SERIAL).

-- 1. Eliminar versiones previas de reembolsar_pedido_rpc para evitar ambigüedades
DO $$ 
DECLARE 
    r RECORD;
BEGIN
    FOR r IN (
        ...
        WHERE proname = 'reembolsar_pedido_rpc' AND pronamespace = 'public'::regnamespace
    ) LOOP
        EXECUTE 'DROP FUNCTION ' || r.sig;
    END LOOP;
END $$;

-- 2. Recrear la función con p_pedido_id de tipo INT
...
BEGIN
    ...
    -- 2. Determinar el monto del reembolso según la moneda
    ...
    -- 3. Asegurar que exista la billetera del cliente
    SELECT EXISTS (
    ...
    -- 4. Acreditar a la billetera correspondiente
    IF p_moneda = 'bs' THEN
    ...
    -- 5. Registrar la transacción en el historial de la billetera (referencia_id es TEXT)
    ...
    -- 6. Actualizar el estado del pedido y marcar la bandera reembolso_billetera
