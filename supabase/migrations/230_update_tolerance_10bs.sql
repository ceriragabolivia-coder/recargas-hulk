-- Migration: 230_update_tolerance_10bs.sql
-- Description: Aumenta la tolerancia de auto-verificación a 10 Bs para pedidos y recargas.
...
BEGIN
    ...
    -- 2. Verificar que el monto coincida (margen de error 10.0 Bs)
    IF ABS(v_apk_pago.monto - p_monto) > 10.0 THEN
        RETURN jsonb_build_object('success', false, 'message', 'El monto del pago APK no coincide dentro de la tolerancia de 10 Bs.');
    END IF;
    ...
    IF v_pedido.pago_verificado IS NOT TRUE AND v_pedido.referencia_pago != 'N/A' AND v_pedido.referencia_pago NOT LIKE 'PAGO_BILLETERA%' THEN
        ...
        BEGIN
            ...
            IF FOUND THEN
                ...
                -- USANDO TOLERANCIA DE 10.0 Bs
                IF ABS(v_apk_pago.monto - v_amount_to_check) <= 10.0 THEN
...
BEGIN
    ...
    IF NEW.status = 'disponible' THEN
        ...
        IF FOUND THEN
            ...
            IF ABS(NEW.monto - v_amount_to_check) <= 10.0 THEN
        ...
        IF FOUND THEN
            -- Verificar si el monto coincide con una tolerancia de 10.0 Bs
            IF ABS(NEW.monto - v_recarga.monto) <= 10.0 THEN
