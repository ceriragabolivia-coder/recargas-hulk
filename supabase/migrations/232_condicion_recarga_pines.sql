-- Migration 232: Condicion de recarga de los ultimos 30 dias para canjear pines
...
DECLARE
    ...
    v_pedidos_exitosos INTEGER;
BEGIN
    -- Validar que el usuario tenga al menos 1 pedido completado en los ultimos 30 dias
    SELECT COUNT(*)
    INTO v_pedidos_exitosos
    FROM pedidos
    WHERE auth_user_id = p_user_id
      AND estado = 'completado'
      AND created_at >= (NOW() - INTERVAL '30 days');

    IF v_pedidos_exitosos = 0 THEN
        RETURN json_build_object(
            'success', false, 
            'message', 'Para canjear pines necesitas haber hecho una recarga exitosamente en los últimos 30 días.'
        );
    END IF;

    -- Obtener la configuracion del tiempo de espera en SEGUNDOS (default: 300 = 5 minutos)
    ...
    IF v_cooldown_segundos > 0 THEN
        -- Verificar el cooldown comparando con el ultimo canje
        ...
        IF v_ultima_recarga IS NOT NULL AND (v_ultima_recarga + (v_cooldown_segundos * interval '1 second')) > NOW() THEN
            -- Calcular cuanto tiempo falta exactamente
    ...
    -- Actualizar saldo segun la moneda
    ...
    -- Marcar pin como canjeado guardando el id de transaccion
    UPDATE pines 
