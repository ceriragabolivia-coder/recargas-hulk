...
BEGIN
    ...
    IF v_is_admin THEN
        ...
        WHERE pago_verificado IS NULL AND estado NOT IN ('cancelado', 'reembolsado', 'completado', 'rechazado');
    ELSE
        SELECT COUNT(*) INTO v_pedidos_verificando FROM public.pedidos 
        WHERE pago_verificado IS NULL AND estado NOT IN ('cancelado', 'reembolsado', 'completado', 'rechazado') AND owner_id = v_owner_id;
    END IF;
    ...
    IF v_is_admin THEN
        ...
        WHERE pago_verificado = true AND estado NOT IN ('completado', 'cancelado', 'reembolsado', 'rechazado');
    ELSE
        SELECT COUNT(*) INTO v_pedidos_reembolso FROM public.pedidos 
        WHERE pago_verificado = true AND estado NOT IN ('completado', 'cancelado', 'reembolsado', 'rechazado') AND owner_id = v_owner_id;
    END IF;
...
GRANT EXECUTE ON FUNCTION public.get_admin_counts_rpc(UUID) TO authenticated, anon;
