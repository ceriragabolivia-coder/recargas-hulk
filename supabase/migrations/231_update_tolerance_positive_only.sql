-- Migration: 231_update_tolerance_positive_only.sql
-- Description: Actualiza la tolerancia para permitir +10 Bs (solo hacia arriba). No se aceptan pagos por debajo del monto.
...
BEGIN
    ...
    -- 2. Verificar que el monto coincida (margen de error +10.0 Bs hacia arriba)
    v_dif := v_apk_pago.monto - p_monto;
    IF v_dif < 0 OR v_dif > 10.0 THEN
        RETURN jsonb_build_object('success', false, 'message', 'El monto del pago APK no coincide. El pago fue menor al requerido o superó la tolerancia máxima.');
    END IF;
    ...
    IF v_pedido.pago_verificado IS NOT TRUE AND v_pedido.referencia_pago != 'N/A' AND v_pedido.referencia_pago NOT LIKE 'PAGO_BILLETERA%' THEN
        ...
        BEGIN
            ...
            IF FOUND THEN
                ...
                -- USANDO TOLERANCIA DE +10.0 Bs hacia arriba
                v_dif := v_apk_pago.monto - v_amount_to_check;
                IF v_dif >= 0 AND v_dif <= 10.0 THEN
...
BEGIN
    ...
    IF NEW.status = 'disponible' THEN
        ...
        IF FOUND THEN
            ...
            -- Verificar si el monto coincide con una tolerancia de +10.0 Bs (hacia arriba únicamente)
            v_dif := NEW.monto - v_amount_to_check;
            IF v_dif >= 0 AND v_dif <= 10.0 THEN
        ...
        IF FOUND THEN
            -- Verificar si el monto coincide con una tolerancia de +10.0 Bs
            v_dif := NEW.monto - v_recarga.monto;
            IF v_dif >= 0 AND v_dif <= 10.0 THEN
