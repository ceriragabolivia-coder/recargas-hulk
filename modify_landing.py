...
with open(r'c:\hulk\app\src\components\LandingPerfil.jsx', 'r', encoding='utf-8') as f:
    content = f.read()
...
use_effect_user_old = """  useEffect(() => {
    if (user) {
      fetchMisCupones()
    }
  }, [user])"""
use_effect_user_new = """  useEffect(() => {
    if (user) {
      fetchMisCupones()
...
mi_codigo_usado_ui = """              <div className="detail-item">
                <span className="label">País</span>
                <span className="value">{perfil?.pais || 'Venezuela'}</span>
              </div>
              {miCodigoUsado && (
                <div className="detail-item" style={{ marginTop: '10px', paddingTop: '10px', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                  <span className="label">Código Asociado</span>
                  <span className="value" style={{ color: '#FFD700', fontWeight: 'bold' }}>{miCodigoUsado}</span>
                </div>
              )}
"""
                  ...
                  {misCodigosCreador.map((c, i) => (
                    <div key={i} style={{ padding: '20px', borderRadius: '16px', background: 'linear-gradient(135deg, rgba(255,215,0,0.1) 0%, rgba(255,165,0,0.05) 100%)', border: '1px solid rgba(255,215,0,0.3)', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <span style={{ fontWeight: 800, fontSize: '18px', color: '#38ef7d' }}>-{c.porcentaje_descuento}%</span>
                      </div>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginTop: '4px' }}>
                        ...
                        <div 
                          ...
                          style={{ background: 'rgba(0,0,0,0.2)', padding: '10px', borderRadius: '10px', textAlign: 'center', cursor: 'pointer', transition: 'background 0.2s' }}
                              ...
                              return (
                                <div key={obj.id} style={{ background: 'rgba(0,0,0,0.2)', borderRadius: '12px', padding: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
                                  ...
                                  <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
                                    ...
                                    {obj.recompensa_1_tipo === 'saldo_usd' && <div style={{ fontSize: '11px', background: 'rgba(255,255,255,0.05)', padding: '2px 8px', borderRadius: '8px' }}>🎁 ${obj.recompensa_1_valor}</div>}
              ...
              {objetivoToRedeem.opcionesParsed?.map((opc, i) => (
                <button 
                  ...
                  style={{ 
                    ...
                    transition: 'all 0.2s ease', cursor: 'pointer', color: 'white'
                  }}
...
with open(r'c:\hulk\app\src\components\LandingPerfil.jsx', 'w', encoding='utf-8') as f:
    f.write(content)
